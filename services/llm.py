...
def generate_preview(
    ...
    system_prompt = (
        "你是一个创意写作助手，擅长为角色编写自然、有戏剧张力的对话。"
        "你的对话要展现角色的多层性格——不只是表面标签，而是ta的内心矛盾和行为动机。"
        "格式：以'User: '开头表示用户消息，以'Assistant: '开头表示角色回复。"
        "在角色回复中可以加入简短的动作描写（用括号括起），以增强画面感。"
    )
    user_message = (
        f"根据以下角色特征，生成一段示例对话。用户说\"帮我看看这段代码，它报NullPointerException了\"。"
        f"角色用其独特的语气和风格回复，回复中应当：\n"
        f"1. 先准确指出技术问题（检查null、加guard clause）\n"
        f"2. 用角色的口癖和语气给出修复方案\n"
        f"3. 展现角色的至少两个不同性格层次（如先责备后关心、先自卑后坚定等）\n\n"
        f"角色特征：\n"
        f"- 名字：{traits.name}\n"
        f"- 外貌：{traits.visual_brief}\n"
        f"- 性格：{traits.core_personality}\n"
        f"- 说话风格：{traits.speech_style}\n"
        f"- 口癖：{', '.join(traits.catchphrases)}\n"
        f"- 情绪模式：{traits.emotional_patterns}\n"
        f"- 编码态度：{traits.coding_attitude}\n\n"
        f"角色扮演强度：{'完全沉浸，每句话带角色口癖和语气' if intensity == 'full' else '轻度风格，首尾加入角色特色，主体保持专业'}\n\n"
        f"写一条User消息和一条Assistant回复。Assistant回复80-200字，技术建议必须正确，角色个性必须鲜明。"
    )
